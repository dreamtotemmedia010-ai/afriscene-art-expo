...
const POEM = "Thoughts on riot, whose ever sighted/ Solo or teams invited, single physiques get Gemini divided/ I'm dark knighted, mind laser-guided/ Big Bang's omen was an mcee and me collided/ My level to find it/ Dark matter stellar chariot Aries riding it/ Scribed, in sacred books slighted/ I was spitting hair eyes & mic ignited/!";
    ...
    <div className="min-h-screen bg-background flex flex-col animate-fade-in-slow">
      ...
      <div className="flex-1 flex flex-col gap-8 p-6 sm:p-10 max-w-6xl mx-auto w-full">
        <div className="flex flex-col sm:flex-row gap-6 items-stretch">
          <div className="sm:w-1/2 aspect-[4/3] rounded-lg border border-gold/20 overflow-hidden cursor-pointer select-none relative group"
            onClick={toggleAudio}
            role="button"
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/20 transition-colors">
              <span className="shimmer-gold font-display text-4xl sm:text-6xl opacity-80">
                {playing ? "❚❚" : "▶"}
              </span>
            </div>
          </div>
          <div className="sm:w-1/2 flex items-center min-w-0">
            <div className="w-full p-4 bg-card rounded-lg border border-border min-h-[120px] flex flex-col items-center justify-center gap-3 min-w-0">
              <p className="shimmer-gold font-display text-2xl sm:text-4xl font-bold tracking-wider text-center break-words max-w-full">
                Interactive Poetry (tap to play)
              </p>
        ...
        <p className="shimmer-gold font-display text-xl sm:text-2xl tracking-widest text-center mt-4">
          Tap image
        </p>
      </div>
      ...
      <div className="w-full pb-6 px-6">
        <p className="text-muted-foreground text-xs tracking-widest font-body text-center">Demo</p>
      </div>
    </div>
  );
};
